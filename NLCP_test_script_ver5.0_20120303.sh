...
#TARGET_BT_SCRIPT=/system/lib/firmware/TIInit_10.6.15.bts
...
TMP=$(calibrator plt calibrate single | busybox grep "Fail" | busybox sed 's/ to.*$//g')
if [ "$TMP" == "Fail" ]; then
fail
break
fi
